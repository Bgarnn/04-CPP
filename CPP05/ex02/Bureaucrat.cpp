...
void Bureaucrat::executeForm(AForm const &paper)
{
	try
	{
		std::cout << YELLOW << this->_name << " executed " << paper.getName() << DEFAULT << std::endl;
		paper.execute(*this);
	}
	catch(const std::exception& e)
	{
		std::cout << RED << this->_name << " couldn't execute " << paper.getName() << " because " << e.what() << DEFAULT << std::endl;
	}
